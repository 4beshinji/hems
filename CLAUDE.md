...
- `hems/sensors/{zone}/{device_type}/{device_id}/{channel}` — 物理センサ telemetry + camera/activity(perception) **[canonical — W3.8c]**
- ~~`office/{zone}/{device_type}/{device_id}/{channel}`~~ — 旧 prefix(W3.8c で削除。task_report のみ `office/{zone}/task_report/{id}` として継続)
