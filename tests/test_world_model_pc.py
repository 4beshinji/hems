...
class TestWorldModelPCRouting:
    ...
    def test_cpu_metrics_update(self, world_model):
        ...
        assert world_model.digital.pc_state.cpu.usage_percent == 42.5
        assert world_model.digital.pc_state.cpu.core_count == 8
        assert world_model.digital.pc_state.cpu.last_update > 0
    ...
    def test_memory_metrics_update(self, world_model):
        ...
        assert world_model.digital.pc_state.memory.used_gb == 12.5
        assert world_model.digital.pc_state.memory.total_gb == 32.0
        assert world_model.digital.pc_state.memory.percent == 39.1
    ...
    def test_gpu_metrics_update(self, world_model):
        ...
        gpu = world_model.digital.pc_state.gpu
    ...
    def test_disk_metrics_update(self, world_model):
        ...
        disk = world_model.digital.pc_state.disk
    ...
    def test_temperature_update(self, world_model):
        ...
        assert world_model.digital.pc_state.cpu.temp_c == 55.0
        assert world_model.digital.pc_state.gpu.temp_c == 68.0
    ...
    def test_process_list_update(self, world_model):
        ...
        procs = world_model.digital.pc_state.top_processes
        assert len(procs) == 2
...
class TestWorldModelPCThresholds:
    ...
    def test_cpu_high_event(self, world_model):
        ...
        events = [e for e in world_model.digital.pc_state.events if e.event_type == "pc_cpu_high"]
        assert len(events) == 1
    ...
    def test_memory_high_event(self, world_model):
        ...
        events = [e for e in world_model.digital.pc_state.events if e.event_type == "pc_memory_high"]
        assert len(events) == 1
    ...
    def test_gpu_hot_event(self, world_model):
        ...
        events = [e for e in world_model.digital.pc_state.events if e.event_type == "pc_gpu_hot"]
        assert len(events) == 1
    ...
    def test_disk_high_event_from_partition(self, world_model):
        ...
        events = [e for e in world_model.digital.pc_state.events if e.event_type == "pc_disk_high"]
        assert len(events) == 1
