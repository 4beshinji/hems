...
>
> **HEMS 連携の前提**: 本ガイドの「HEMS実現機能」は `docker compose --profile ha up` + `HA_URL`/`HA_TOKEN`/`HEMS_HA_ENTITY_MAP` 設定、または `docker compose --profile zigbee up` + `control_actuator` 等が必要です。実際のセットアップ手順は `docs/SMART_HOME_SETUP.md` を参照してください。
...
- **HEMS実現機能**:
  ...
  - 電力モニタリング → WorldModel に消費電力反映（洗濯完了検知には振動センサーを推奨）
...
- **技適**: 未確認
- **HEMS実現機能**:
  - （現状 `lock` ドメインは `ha-bridge` の追跡対象外のため、施錠状態の反映・自動施錠は未対応）
