...
| 到着前HVAC | Rule 2 | 全て |
| 疲労時減光 | Rule 11 | 全て |
| シーン実行 | `execute_scene` | 全て |
| センサー値取得 | `get_sensor_data` | 全て |
| 不在モード | RuleEngine (防犯照明ランダム点灯) | 全て |
| ゲストモード | `set_guest_mode` (自動化一時停止) | 全て |
| 天気連動 | `get_weather` + RuleEngine (降雨/猛暑) | 全て |
| サーカディアンリズム照明 | RuleEngine (時間帯色温度自動変化) | 全て |

全機能実装済み。エネルギーダッシュボード（電力消費の可視化）は Devices ページ（`/devices`）に統合。
