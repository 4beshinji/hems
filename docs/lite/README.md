...
> *C5 のみ例外的に 5 分クールダウンを適用 (不在状態が続く限り繰り返し通知されるのを抑制)。
...
複数プロバイダを同時設定可能。設定されたすべてのプロバイダに順次送信される。
...
レベルは表示ラベルに変換される: CRITICAL→`[CRITICAL]`, HIGH→`[HIGH]`, NORMAL→`[INFO]`, INFO→`[REPORT]`
...
| 変数 | デフォルト | 説明 |
|------|----------|------|
| `HEMS_PORT_MQTT` | `1893` | MQTT ホスト公開ポート |
| `HEMS_PORT_NOTIFIER` | `8019` | Notifier ホスト公開ポート |
| `HEMS_PORT_BIOMETRIC_BRIDGE` | `8017` | Biometric Bridge ホスト公開ポート |
| `HEMS_PORT_PERCEPTION` | `8018` | Perception ホスト公開ポート |
| `HEMS_PORT_HA_BRIDGE` | `8016` | HA Bridge ホスト公開ポート |
| `MQTT_BIND_ADDR` | `0.0.0.0` | MQTT バインドアドレス |

#### MQTT

| 変数 | デフォルト | 説明 |
|------|----------|------|
| `MQTT_HOST` | `mosquitto` | MQTTブローカーホスト (コンテナ内) |
| `MQTT_PORT` | `1883` | MQTTポート (コンテナ内) |
...
| `NOTIFIER_URL` | `http://notifier:8000` | Notifier サービスURL (コンテナ内) |
...
#### Biometric Bridge (profile: biometric)

| 変数 | デフォルト | 説明 |
|------|----------|------|
| `BIOMETRIC_WEBHOOK_SECRET` | `changeme` | Gadgetbridge Webhook HMAC シークレット |
| `BIOMETRIC_PROVIDER` | `gadgetbridge` | バイオメトリクスプロバイダ |

#### Perception (profile: perception)

| 変数 | デフォルト | 説明 |
|------|----------|------|
| `HEMS_PERCEPTION_CAMERAS` | `[]` | カメラ設定 (JSON配列) |
| `HEMS_PERCEPTION_POSE_MODEL` | RTMO `rtmo-s` URL | ポーズモデル (rtmlib/ONNX, Apache-2.0) |
| `HEMS_PERCEPTION_DEVICE` | `cpu` | 推論デバイス: `cpu` or `cuda` |
| `HEMS_PERCEPTION_CONFIDENCE` | `0.4` | 検出信頼度閾値 |
| `HEMS_PERCEPTION_INTERVAL` | `30` | 推論間隔 (秒) |

#### Home Assistant (profile: ha)

| 変数 | デフォルト | 説明 |
|------|----------|------|
| `HA_URL` | `http://host.docker.internal:8123` | Home Assistant URL |
| `HA_TOKEN` | (空) | HA 長期アクセストークン |
...
データ保持期間: `SentinelDB.prune_old_data(retention_days=90)` で 90 日以前のデータを削除可能。
現在は自動実行されないため、必要に応じて手動またはスケジューラから呼び出すこと。
...
| `hems/sensors/+/camera/+/status` | カメラ検知 (人数, 姿勢) |
| `hems/sensors/+/activity/+` | 活動レベル |
| `hems/sensors/+/sensor/#` | 環境センサー (温湿度, CO2, etc.) |
...
// hems/sensors/living_room/camera/cam01/status
{"person_count": 1, "posture": "sitting", "activity_level": 0.15, "posture_duration_sec": 2700}

// hems/sensors/living_room/sensor/esp32_01/temperature
{"value": 24.5}
```
...
- `rtmo-s` (既定) は CPU でも軽量。さらに軽くする場合は `HEMS_PERCEPTION_POSE_MODEL` を `rtmo-t` 系 URL に変更
