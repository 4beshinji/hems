...
from loaders import get_loader, DocumentEntry
from config import DEFAULT_EXTENSIONS, DEFAULT_EXCLUDE_PATTERNS, RRF_K
...
_STOP_WORDS = frozenset({
    ...
    "self", "def", "class", "import", "return", "none", "true", "false",
})
...
class DocumentIndex:
    ...
    def __init__(self, embedding_client: Optional[EmbeddingClient] = None):
        ...
        # BM25 (title)
        self._title_bm25: Optional[BM25Okapi] = None
    ...
    def add_source(self, name: str, path: str, extensions: list[str] | None = None,
        ...
        if not source_path.exists():
            ...
            self._sources[name] = {
                "path": path, "extensions": extensions or DEFAULT_EXTENSIONS,
    ...
    async def search(self, query: str, source: str | None = None,
                     doc_type: str | None = None, tags: list[str] | None = None,
                     path_prefix: str | None = None, max_results: int = 10) -> list[dict]:
        ...
        if not has_query:
            ...
            now = time.time()
            items = [(k, 1.0 + (RECENT_BOOST - 1.0 if now - self.documents[k].modified_at < RECENT_WINDOW else 0))
                     for k in candidates]
        ...
        if query_tokens and self._bm25 is not None:
            ...
            bm25_ranked = [
                self._doc_keys[i]
                for i in sorted(range(len(self._doc_keys)),
        ...
        if has_query and self._embedder and self._embedder.available and self._vectors.size > 0:
            ...
            if query_vec is not None:
                vec_results = self._vectors.search(
                    query_vec, top_k=max_results * 3, allowed_keys=candidate_set,
                )
        ...
        if query_tokens and self._title_bm25 is not None:
            ...
            title_ranked = [
                self._doc_keys[i]
                for i in sorted(range(len(self._doc_keys)),
                                key=lambda i: title_scores[i], reverse=True)
    ...
    def get_recent(self, limit: int = 10, source: str | None = None) -> list[dict]:
        ...
        return [
            {
                "source": d.source_name, "path": d.path, "title": d.title,
                "doc_type": d.doc_type, "tags": d.tags,
                "modified_at": d.modified_at, "word_count": d.word_count,
            }
    ...
    def get_source_stats(self) -> list[dict]:
        ...
        for name, src in self._sources.items():
            ...
            stats.append({
                "name": name, "path": src["path"], "active": src["active"],
                "doc_count": doc_count, "type_counts": type_counts,
    ...
    def _filter_candidates(self, source: str | None, doc_type: str | None,
                           tags: list[str] | None, path_prefix: str | None) -> list[str]:
    ...
    def _build_results(self, scored_keys: list[tuple[str, float]]) -> list[dict]:
        ...
        for doc_key, score in scored_keys:
            ...
            results.append({
                "source": entry.source_name, "path": entry.path,
                "title": entry.title, "doc_type": entry.doc_type,
                "tags": entry.tags, "score": round(score, 4),
                "snippet": snippet, "modified_at": entry.modified_at,
