...
from sentinel.escalation import EscalationVerdict, Escalator
from sentinel.gray_zone import GrayZoneDetector, GrayZoneEvent
from sentinel.rules import Alert, AlertLevel, RuleEngine
...
class Sentinel:
    ...
    def _setup_mqtt(self):
        self._mqtt = MqttPublisher(
            ...
            client_id=f"sentinel-{SITE_ID}",
        )
        self._mqtt.set_message_callback(self._on_mqtt_message)

        topics = [
            "hems/personal/biometrics/#",
            "hems/sensors/+/camera/+/status",
            "hems/sensors/+/activity/+",
            "hems/sensors/+/sensor/#",
            "hems/+/bridge/status",
    ...
    def _on_mqtt_message(self, topic: str, payload: dict):
        if self._loop is None or self._loop.is_closed():
            return
        try:
            self._loop.call_soon_threadsafe(self._mqtt_queue.put_nowait, (topic, payload))
    ...
    def _handle_mqtt(self, topic: str, payload: bytes | dict):
        ...
        # Camera status: hems/sensors/{zone}/camera/{cam_id}/status
        elif len(parts) >= 6 and parts[3] == "camera" and parts[5] == "status":
            zone_id = parts[2]
        ...
        # Activity: hems/sensors/{zone}/activity/{monitor_id}
        elif len(parts) >= 5 and parts[3] == "activity":
            zone_id = parts[2]
            self.state.update_activity(
                zone_id=zone_id,
                person_count=data.get("person_count", 0),
        ...
        # Environment: hems/sensors/{zone}/sensor/{device_id}/{channel}
        elif len(parts) >= 6 and parts[3] == "sensor":
            zone_id = parts[2]
            channel = parts[5]
    ...
    def _handle_biometric(self, metric: str, data: dict):
        ...
        if metric == "heart_rate":
            ...
            value = data.get("value", data.get("rmssd_ms", data.get("celsius", data.get("breaths_per_minute"))))
            if value is not None:
    ...
    async def _send_notification(self, level: str, title: str, body: str, source: str = "", zone: str = ""):
    ...
    async def _summary_scheduler(self):
        ...
        while self._running:
            ...
            if now.hour == target_hour and now.minute == target_min and self._last_summary_date != today:
    ...
    async def _send_daily_summary(self):
        ...
        if s.sleep.duration_hours > 0:
            lines.append(f"Sleep: {s.sleep.duration_hours:.1f}h (quality: {s.sleep.quality_score}/100)")
        ...
        for zid, zone in s.zones.items():
            ...
            if env.temperature is not None:
                lines.append(
                    f"{zid}: {env.temperature:.1f}C / {env.humidity:.0f}%"
                    if env.humidity
                    else f"{zid}: {env.temperature:.1f}C"
                )
