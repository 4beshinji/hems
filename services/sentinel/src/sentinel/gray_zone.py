...
from sentinel.config import (
    ...
    FATIGUE_HIGH,
    GRAY_ZONE_FACTOR,
    GRAY_ZONE_MIN_SIGNALS,
    HR_HIGH,
    PATTERN_DEVIATION_MINUTES,
...
class GrayZoneDetector:
        ...
        if len(hr_hist) >= 6:  # need enough data points
            ...
            if early and late:
                ...
                if late_avg > early_avg * 1.15:  # 15% increase
                    signals.append(f"HR上昇傾向: {early_avg:.0f}→{late_avg:.0f}bpm (過去{TREND_WINDOW_HOURS}h)")
        ...
        if len(spo2_hist) >= 6:
            ...
            if early and late:
                ...
                if late_avg < early_avg - 2:  # 2% drop
                    signals.append(f"SpO2低下傾向: {early_avg:.0f}→{late_avg:.0f}% (過去{TREND_WINDOW_HOURS}h)")
        ...
        if len(stress_hist) >= 6:
            ...
            if early and late:
                ...
                if late_avg > early_avg * 1.3:
                    signals.append(f"ストレス上昇傾向: {early_avg:.0f}→{late_avg:.0f} (過去{TREND_WINDOW_HOURS}h)")
    ...
    def _check_contradictions(self, s: OccupantState) -> GrayZoneEvent | None:
        ...
        if (
            ...
            signals.append(f"日中臥位 + HR={s.heart_rate.value:.0f}bpm")

        # High activity level + low HR (sensor malfunction or concerning)
        if (
            s.activity.activity_level > 0.5
            and s.heart_rate.is_fresh
            and s.heart_rate.value is not None
            and s.heart_rate.value < 55
        ):
            signals.append(f"高活動(level={s.activity.activity_level:.1f}) + 低HR={s.heart_rate.value:.0f}bpm")
        ...
        if (
            ...
            signals.append(f"在室中だがバイタル{s.heart_rate.age_seconds / 60:.0f}分途絶")
    ...
    def _check_sensor_gap(self, s: OccupantState) -> GrayZoneEvent | None:
        ...
        if signals:
            return GrayZoneEvent(
                ...
                confidence=0.5,
                data={
                    "biometric_age_s": s.heart_rate.age_seconds,
    ...
    def _check_behavior_deviation(self, s: OccupantState) -> GrayZoneEvent | None:
        ...
        if (
            ...
            if hour_f > avg_wake + PATTERN_DEVIATION_MINUTES / 60:
                signals.append(f"通常{avg_wake:.1f}時に起床だが{hour_f:.1f}時も臥位")

        # Baseline HR deviation
        if (
            self._hr_baseline > 0
            and self._hr_baseline_count >= 20
            and s.heart_rate.is_fresh
            and s.heart_rate.value is not None
        ):
            deviation = abs(s.heart_rate.value - self._hr_baseline)
            if deviation > self._hr_baseline * 0.25:  # >25% from baseline
                signals.append(f"HR基準値{self._hr_baseline:.0f}から{deviation:.0f}bpm乖離")
    ...
    def _update_baselines(self, s: OccupantState):
        ...
        hour = datetime.now().hour
        if 5 <= hour <= 12 and s.activity.posture in ("standing", "walking") and s.sleep.stage == "awake":
