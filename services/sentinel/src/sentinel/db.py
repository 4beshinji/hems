...
class SentinelDB:
    ...
    async def log_metric(self, timestamp: float, metric: str, value: float, zone: str = ""):
        await self._db.execute(
            "INSERT INTO metrics_history (timestamp, metric, value, zone) VALUES (?, ?, ?, ?)",
    ...
    async def save_daily_summary(self, date_str: str, summary_json: str, sent: bool = False):
        await self._db.execute(
            "INSERT OR REPLACE INTO daily_summary (date, summary_json, sent) VALUES (?, ?, ?)",
    ...
    async def get_metric_history(self, metric: str, since_ts: float) -> list[tuple[float, float]]:
        rows = await self._db.execute_fetchall(
            "SELECT timestamp, value FROM metrics_history WHERE metric = ? AND timestamp >= ? ORDER BY timestamp",
            (metric, since_ts),
        )
