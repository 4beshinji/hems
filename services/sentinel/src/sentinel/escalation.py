...
from sentinel.config import (
    ...
    LLM_API_URL,
    LLM_DAILY_BUDGET,
    LLM_ESCALATION_COOLDOWN,
    LLM_MODEL,
    LLM_PROVIDER,
    SITE_NAME,
)
from sentinel.gray_zone import GrayZoneEvent
...
class Escalator:
    ...
    async def evaluate(self, event: GrayZoneEvent, state: OccupantState) -> EscalationResult | None:
        ...
        try:
            ...
            logger.info(
                f"LLM escalation: pattern={event.pattern}, verdict={result.verdict}, budget={self.budget_remaining}"
            )
            return result
