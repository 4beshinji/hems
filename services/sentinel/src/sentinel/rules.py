...
from sentinel.state import OccupantState
...
class RuleEngine:
    ...
    def _evaluate_biometric_critical(self, s: OccupantState) -> list[Alert]:
        ...
        if s.spo2.is_fresh and s.spo2.value is not None and s.spo2.value < SPO2_CRITICAL:
            if self._check("C1_spo2", COOLDOWN_CRITICAL):
                alerts.append(
                    Alert(
                        level=AlertLevel.CRITICAL,
                        rule_id="C1",
                        title="血中酸素が危険レベル",
                        body=f"SpO2が{s.spo2.value:.0f}%です（閾値: {SPO2_CRITICAL}%）。直ちに確認してください。",
                        source="biometric",
    ...
    def _evaluate_biometric_high(self, s: OccupantState) -> list[Alert]:
        ...
        is_night = (
            night_start <= now <= night_end if NIGHT_START_H < NIGHT_END_H else (now >= night_start or now <= night_end)
        )
        if is_night and s.activity.posture == "walking" and s.activity.person_count > 0:
        ...
        # H7: Body temp high
        if s.body_temp.is_fresh and s.body_temp.value is not None and s.body_temp.value > BODY_TEMP_HIGH:
        ...
        if (
            s.respiratory_rate.is_fresh
            and s.respiratory_rate.value is not None
            ...
            if self._check("H8_resp_high", COOLDOWN_HIGH):
                alerts.append(
                    Alert(
                        ...
                        rule_id="H8",
                        title="呼吸数上昇",
                        body=f"呼吸数が{s.respiratory_rate.value:.0f}回/分です（閾値: {RESPIRATORY_RATE_HIGH}回/分）。",
                        source="biometric",
    ...
    def _evaluate_biometric_normal(self, s: OccupantState) -> list[Alert]:
        ...
        if s.stress.is_fresh and s.stress.value is not None and s.stress.value > STRESS_HIGH:
        ...
        if s.fatigue.is_fresh and s.fatigue.value is not None and s.fatigue.value > FATIGUE_HIGH:
            if self._check("N_fatigue", COOLDOWN_NORMAL):
                alerts.append(
                    Alert(
                        ...
                        rule_id="N_fatigue",
                        title="疲労蓄積",
                        body=f"疲労スコアが{s.fatigue.value:.0f}です（閾値: {FATIGUE_HIGH}）。休息を取りましょう。",
                        source="biometric",
                        data={"fatigue": s.fatigue.value},
                    )
                )

        if s.hrv.is_fresh and s.hrv.value is not None and s.hrv.value < HRV_LOW:
            if self._check("N_hrv_low", COOLDOWN_NORMAL):
                alerts.append(
                    Alert(
                        ...
                        rule_id="N_hrv",
                        title="HRV低下",
                        body=f"HRVが{s.hrv.value:.0f}msです（閾値: {HRV_LOW}ms）。自律神経の疲労が出ています。",
                        source="biometric",
    ...
    def _evaluate_info(self, s: OccupantState) -> list[Alert]:
        ...
        if hour >= 20 and s.activity.steps > 0 and s.activity.steps < 1000:
