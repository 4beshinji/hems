...
import { usePsdIdle } from './usePsdIdle'
...
export function usePsdState(): PsdAvatarState {
  ...
  return useMemo<PsdAvatarState>(() => ({
    ...
    // 腕: アイドルポーズ
    armLeft: idle.armLeft,
    armRight: idle.armRight,

    // 衣装: イベント駆動 > デフォルト
    costume: eventDelta.costume ?? DEFAULT_STATE.costume,

    // FX: デフォルトにイベント差分をスプレッドマージ
  ...
  }), [mouth, toneExpr, eventDelta, idle])
}
