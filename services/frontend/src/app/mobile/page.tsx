...
import { Plus, RefreshCw, Smartphone, Trash2, MapPin } from 'lucide-react'
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-5xl mx-auto">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          コンパニオン端末の管理と、geofence リマインド用の登録地点
        </p>
      </header>
...
function DevicesSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      />
      <QrDialog response={qrTarget} onClose={() => setQrTarget(null)} />
    </div>
  )
}
...
function QrDialog({
  ...
  return (
    <Dialog open={response !== null} onOpenChange={(v) => { if (!v) onClose() }}>
...
function PlacesSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      <PlaceEditDialog open={dialogOpen} onOpenChange={setDialogOpen} place={editing} />
    </div>
  )
}
...
function PlaceEditDialog({
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (place) return updateFrequentPlace(place.id, form)
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="半径 (m)">
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, radius_m: parseInt(e.target.value) || 200 }))}
                className="w-full border rounded-md px-3 py-1.5 text-sm"
              />
            </Field>
            <Field label="クールダウン (分)">
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, cooldown_min: parseInt(e.target.value) || 60 }))}
                className="w-full border rounded-md px-3 py-1.5 text-sm"
              />
            </Field>
          </div>
